...
tyran.roar = function() {
  return `${this.objName} says RAWERSRARARWERSARARARRRR`;
};
console.log(tyran.roar());

// ==== Arrays ====

// Given an array of college graduates.  Complete the following requests using any
// array method you like
...
universities in the graduates array. This will be an array of strings.

Once you have the new array created, sort the universities alphabetically and log the result. */
const universities = [];
graduates.map(item => universities.push(item.university));
console.log(universities);

/* Request 2: Create a new array called contactInfo that contains both first name 
and email of each student. This will be an array of strings.
...
const unisWithUni = [];
graduates
  .filter(item => item.university.includes("Uni"))
  .map(item => unisWithUni.push(item.university));
...
zooAnimals
  .filter(item => item.population < 5)
  .map(item => lowPopulationAnimals.push(item));
console.log(lowPopulationAnimals);

/* Request 4: .reduce() 

The zoos need to know their total animal population across the United States. Find the total population 
from all the zoos using the .reduce() method. Remember the reduce method takes two arguments: 
a callback (which itself takes two args), and an initial value for the count.

*/
const countPopulation = (total, item) => {
